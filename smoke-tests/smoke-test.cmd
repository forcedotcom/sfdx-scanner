...
call %EXE_NAME% scanner:run --format junit --target test\code-fixtures\projects\app\force-app --outfile smoke-test-results\pmd-eslint.xml || exit /b 1
echo "==== Run rules against a typescript file, which should run ESLint-Typescript ===="
call %EXE_NAME% scanner:run --format junit --target test\code-fixtures\projects\ts\src\simpleYetWrong.ts --tsconfig test\code-fixtures\projects\tsconfig.json --outfile smoke-test-results\eslint-typescript.xml || exit /b 1
echo "==== Run RetireJS against a folder ===="
call %EXE_NAME% scanner:run --format junit --engine retire-js --target test\code-fixtures\projects\dep-test-app\folder-a --outfile smoke-test-results\retire-js.xml || exit /b 1
echo "==== Run CPD against a folder ===="
call %EXE_NAME% scanner:run --format junit --engine cpd --target test\code-fixtures\projects\cpd-test-app\src\classes --outfile smoke-test-results,cpd.xm || exit /b 1
echo "==== Run PMD with custom config via --pmdconfig flag ===="
call %EXE_NAME% scanner:run --format junit --engine pmd --target test\code-fixtures\projects\app\force-app --pmdconfig test\test-xml\apex\smoke-config.xml --outfile smoke-test-results\pmd-customconfig.xml || exit /b 1
echo "==== Run Salesforce Graph Engine's non-DFA rules against a folder ===="
call %EXE_NAME% scanner:run --format junit --engine sfge --target test\code-fixtures\projects\sfge-smoke-app\src --projectdir test\code-fixtures\projects\sfge-smoke-app\src --outfile smoke-test-results\sfca-pathless.xml || exit /b 1
echo "==== Run Salesforce Graph Engine's DFA rules against a folder ===="
call %EXE_NAME% scanner:run:dfa --format junit --target test\code-fixtures\projects\sfge-smoke-app\src --projectdir test\code-fixtures\projects\sfge-smoke-app\src --outfile smoke-test-results\sfca-dfa.xml || exit /b 1
...
call %EXE_NAME% scanner:run --format junit --category SomeCat1,Security --target test\code-fixtures\projects\app\force-app --outfile smoke-test-results\pmd-custom-rules.xml || exit /b 1
