...
public class StackDepthLimitTest {
    ...
    private static final int STACK_DEPTH_LIMIT_OVERRIDE = 7;
    ...
    public void testStackDepth(String testName, int stackDepthToCreate, boolean expectToReachSystemDebug) {
        // Since we already create two methods other than the dynamic ones, subtract them from required count
        ...
        if (expectToReachSystemDebug) {
            ...
            List<TestRunner.Result<PathVertexVisitor>> paths = TestRunner.get(g, sourceCode).walkPaths();
    ...
    @Test
    public void testValueResolutionClearsDepth() {
        String[] sourceCode = {
            "public class MyClass {\n"
                    + "   public void doSomething() {\n"
                    + "       if (checkDelete()) {\n"
    ...
    private static String getSelfReferenceMethods(String methodName, int count) {
        ...
        final StringBuilder builder = new StringBuilder();
        while (index < count) {
            builder.append(String.format("   public MyClass %s%d() {\n" +
                "       return this;\n" +
                "   }\n", methodName, index++));
        }
        return builder.toString();
