...
public class MethodPathBuilderVisitor {
	...
	private static final String NAME_PROPERTY = "Name_CaseSafe"; // FIXME: identify where this gets created for real
    ...
    public static void apply(GraphTraversalSource g, Vertex methodVertex) {
		...
		// Handle <clint>() methods
		if (methodVertex.value(NAME_PROPERTY).equals(MethodUtil.STATIC_CONSTRUCTOR_CANONICAL_NAME)) {
			handleClintMethod(g, methodVertex);
		} else {
	...
	/**
	 * Handles default method <clint> generated by jorje for every class.
	 * Block statements under <clint> are static code blocks such as
	 * 	class MyClass {
	 * 		static {
	 *			System.debug('hello');
	 *      }
	 *  }
	 * @param g reference to graph
	 * @param methodVertex represents vertex of <clint>()
	 */
	private static void handleClintMethod(GraphTraversalSource g, Vertex methodVertex) {
		final List<Vertex> blockStatements = GremlinUtil.getChildren(g, methodVertex, NodeType.BLOCK_STATEMENT);
		for (Vertex blockStatement : blockStatements) {
			// We should handle each block statement one after another, as if they were in the same method
			// since they will all be executed one after another.
			// TODO: Confirm that static blocks get executed in the same order they are listed in the class
			if (blockStatement != null) {
//				final Vertex staticBlockVertex = CloneUtil.clone(methodVertex);
				final MethodPathBuilderVisitor visitor = new MethodPathBuilderVisitor(g);
				visitor._visit(blockStatement, methodVertex, true);
			}
		}
	}
