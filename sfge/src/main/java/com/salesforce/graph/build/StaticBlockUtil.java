 ...
 * }
 *
 * Having multiple block statements inside a method breaks SFGE's makes handling code blocks in <clint>()
 * impossible. As an alternative, we are creating synthetic vertices in the Graph to represent the static
 * blocks as individual methods. We also create one top-level synthetic method ("StaticBlockInvoker") that invokes individual static
 * block methods. While creating static scope for this class, we should invoke the method call expressions
 * inside the top-level synthetic method.
 *
...
public final class StaticBlockUtil {
	...
	private static final String SYNTHETIC_STATIC_BLOCK_METHOD_NAME = "SyntheticStaticBlock_%d";
	private static final String STATIC_BLOCK_INVOKER_METHOD = "StaticBlockInvoker";
	 ...
	 * @param staticBlockIndex index to use for name uniqueness -
	 *                            TODO: this index is currently just the child count index and does not
	 *                         necessarily follow sequence
	 * @return new synthetic method vertex with name SyntheticStaticBlock_%d, which will be the parent
	 * for blockStatementVertex, and a sibling of <clint>()
	 */
	public static Vertex createSyntheticStaticBlockMethod(GraphTraversalSource g, Vertex clintVertex, Vertex blockStatementVertex,  int staticBlockIndex) {
		...
		addSyntheticStaticBlockMethodProperties(g, definingType, syntheticMethodVertex, staticBlockIndex);
	...
	/**
	 * If rootNode contains synthetic static block methods (created through {@link #createSyntheticStaticBlockMethod}),
	 * adds a top-level synthetic method that invokes each static block method.
	 */
	public static void createSyntheticStaticBlockInvocation(GraphTraversalSource g, Vertex rootNode) {
		...
		if (!staticBlockMethods.isEmpty()) {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Creating synthetic invocation method for {} to invoke {} static block(s)", definingType, staticBlockMethods.size());
			}

			final Vertex invokerMethodVertex = createSyntheticInvocationsMethod(g, rootNode, staticBlockMethods, definingType);
			MethodPathBuilderVisitor.apply(g, invokerMethodVertex);
		}
	}

	static boolean isStaticBlockStatement(JorjeNode node, JorjeNode child) {
		return isClintMethod(node)
			&& containsStaticBlock(node)
			&& isBlockStatement(child);
	}

	private static Vertex createSyntheticInvocationsMethod(GraphTraversalSource g, Vertex rootNode, List<Vertex> staticBlockMethods, String definingType) {
		// Create new synthetic method StaticBlockInvoker to invoke each synthetic static block method
		...
		GremlinVertexUtil.addParentChildRelationship(g, invokerMethodVertex, blockStatementInInvoker);
		...
		for (int i = 0; i < staticBlockMethods.size(); i++) {
			...
			final Vertex exprStaticBlockMethodCall = createMethodCallExpression(g, definingType, staticBlockMethod, isLastMethod);

			// Add the expression statement containing the method call inside StaticBlockInvoker's block statement
			GremlinVertexUtil.addParentChildRelationship(g, blockStatementInInvoker, exprStaticBlockMethodCall);
		}

		return invokerMethodVertex;
	}

	private static Vertex createMethodCallExpression(GraphTraversalSource g, String definingType, Vertex staticBlockMethod, boolean isLastMethod) {
		...
		final Vertex staticBlockMethodCall = g.addV(ASTConstants.NodeType.METHOD_CALL_EXPRESSION).next();
		addMethodCallExpressionProperties(g, definingType, staticBlockMethod, staticBlockMethodCall);
		...
		final Vertex emptyMethodReference = g.addV(ASTConstants.NodeType.EMPTY_REFERENCE_EXPRESSION).next();
		addEmptyMethodReferenceProperties(g, definingType, emptyMethodReference);
		GremlinVertexUtil.addParentChildRelationship(g, staticBlockMethodCall, emptyMethodReference);
		return expressionStmt;
	}

	private static void addExpressionStmtProperties(GraphTraversalSource g, String definingType, Vertex expressionStmt, boolean isLastMethod) {
	...
	private static void addEmptyMethodReferenceProperties(GraphTraversalSource g, String definingType, Vertex emptyMethodReference) {
	...
	private static void addMethodCallExpressionProperties(GraphTraversalSource g, String definingType, Vertex staticBlockMethod, Vertex staticBlockMethodCall) {
	...
	private static void addBlockStatementProperties(GraphTraversalSource g, String definingType, Vertex blockStatementInInvoker) {
	...
	private static void addStaticModifierProperties(GraphTraversalSource g, String definingType, Vertex modifier) {
	...
	private static void addStaticBlockInvokerProperties(GraphTraversalSource g, String definingType, Vertex staticBlockInvokerVertex) {
	...
	private static void addSyntheticStaticBlockMethodProperties(GraphTraversalSource g, String definingType, Vertex syntheticMethodVertex, int staticBlockIndex) {
		verifyType(syntheticMethodVertex, ASTConstants.NodeType.METHOD);
		final Map<String, Object> properties = new HashMap<>();
		properties.put(Schema.NAME, String.format(SYNTHETIC_STATIC_BLOCK_METHOD_NAME, staticBlockIndex));
	...
	private static void addCommonSynthMethodProperties(GraphTraversalSource g, String definingType, Vertex staticBlockInvokerVertex, Map<String, Object> properties) {
	...
	private static void addProperties(GraphTraversalSource g, Vertex vertex, Map<String, Object> properties) {
