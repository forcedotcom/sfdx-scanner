...
export default class Describe extends ScannerCommand {
	...
	async runInternal(): Promise<AnyJson> {
		...
		if (rules.length === 0) {
			...
			this.warn(messages.getMessage('output.noMatchingRules', [this.flags.rulename as string]));
	...
	private logStyledRule(rule: DescribeStyledRule): void {
		const {
			name, engine, runWith, isPilot, enabled, categories,
			rulesets, languages, description
		} = rule;
		const strippedObject = {
			name, engine, runWith, isPilot, enabled, categories,
			rulesets, languages, description
		};
		this.styledObject(strippedObject);
	}
}
