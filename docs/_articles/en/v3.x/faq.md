...
Salesforce Graph Engine needs to build up a context of the source code in its entirety before applying rules to capture violations. Depending on the complexity of code, such as number of conditionals, classes to instantiate, method invocations, etc, some projects may take longer to process than others.

#### Q: I’m looking to verify a single entry point. Is there a shortcut?

(TODO: remove this question if we are not including method type targeting)

You can use --target parameter to execute only your entry method. For example, if you are looking to check method myMethod() in class MyClass, you can use --target value as /path/to/MyClass.cls#myMethod
This would ignore all other entry methods in the execution and get you results for only the entry method you are currently interested in.

#### Q: My code is guarded through a different way than an Apex CRUD/FLS check. Is there a way to suppress violations on it?

Please read about Engine Directives to see how this can be done.

<TODO: information about env variables>


## Questions about interpreting results generated by SFGE

(TODO: should this section be a part of SFGE architecture instead?)
...
*Common Scenario*
[<Validation_Type] validation is missing for [<Operation_Name>] operation on [<Object_Type>] with field(s) [<Comma_Separated_Fields>]
...
*Additional Clause*
[<Validation_Type] validation is missing for [<CRUD_Operation>] operation on [<Object_Type>] with field(s) [<Comma_Separated_Fields>] - SFCA may not have parsed some objects/fields correctly. Please confirm if the objects/fields involved in these segments have FLS checks: [<Unknown_Segments>]

Same as the https://salesforce.quip.com/HWMeAnkwDV2z#temp:C:AaU08589a1c11e24fe9b68ac600e, but this additionally means Salesforce Graph Engine is not confident about the object names and/or field names it detected. This could also happen if the field or object ends with __r. In both cases, please make sure the relational field/object or the unparsed segments has the required CRUD/FLS checks. Once you’ve taken care of it, you could add an https://salesforce.quip.com/HWMeAnkwDV2z#temp:C:AaU2430f0407816163e5502ad5e6 to let Salesforce Graph Engine know that it doesn’t have to create a violation.
...
#### Q: Why am I seeing this violation?

The following are the source, sink, and sanitizer definitions that Salesforce Graph Engine uses. The CRUD operation which has the violation could be missing access check that Salesforce Graph Engine understands.
...
If you determine that the CRUD operation in question is protected by a sanitizer that Salesforce Graph Engine doesn’t recognize, you can https://salesforce.quip.com/HWMeAnkwDV2z#temp:C:AaU2430f0407816163e5502ad5e6 to let Salesforce Graph Engine know that the CRUD operation is in fact safe.
