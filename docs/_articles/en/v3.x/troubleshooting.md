...
Make sure that the entries for eslint and eslint-typescript in `${HOME}/.sfdx-scanner/{{ site.data.versions-v3.configfile }}` both exclude the folder
...
Make the following changes to the PMD engine node in your `${HOME}/.sfdx-scanner/{{ site.data.versions-v3.configfile }}` file to resolve this error.
