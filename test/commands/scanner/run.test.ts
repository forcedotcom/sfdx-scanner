...
describe('scanner:run', function () {
	...
	describe('E2E', () => {
		...
		describe('Dynamic Input', () => {
			describe('Globs', () => {
				describe('Test Case: Running rules against a single positive glob', () => {
					setupCommandTest
						.command(['scanner:run',
				...
				describe('Test Case: Running rules against multiple positive globs', () => {
					setupCommandTest
						.command(['scanner:run',
							// NOTE: When running the command for real, a glob would have to be wrapped in SINGLE-QUOTES. But the tests sidestep that, somehow.
							'--target', 'test/code-fixtures/apex/A*,test/code-fixtures/apex/I*',
							'--ruleset', 'ApexUnit',
							'--format', 'xml'
						])
						.it('Any file matching at least one positive glob is evaluated', ctx => {
							// We'll split the output by the <file> tag first, so we can get each file that violated rules.
							const results = ctx.stdout.split('<result ');
							// The first list item is going to be the header, so we need to pull that off.
							results.shift();
				...
				describe('Test Case: Mixing positive and negative globs', () => {
					setupCommandTest
						.command(['scanner:run',
							// NOTE: When running the command for real, a glob would have to be wrapped in SINGLE-QUOTES. But the tests sidestep that, somehow.
							'--target', 'test/code-fixtures/apex/*Tests.cls,!**/I*.cls,!**/Ab*.cls',
							'--ruleset', 'ApexUnit',
							'--format', 'xml'
						])
						.it('Files must match all negative globs to not be excluded', ctx => {
							// We'll split the output by the <file> tag first, so we can get each file that violated rules.
							const results = ctx.stdout.split('<result ');
							// The first list item is going to be the header, so we need to pull that off.
							results.shift();
							// Verify that each set of violations corresponds to the expected file.
							expect(results.length).to.equal(1, 'Only one file should have been evaluated');
							expect(results[0]).to.match(/file="test(\/|\\)code-fixtures(\/|\\)apex(\/|\\)AccountServiceTests.cls"/);
