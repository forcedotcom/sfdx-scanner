	...
	describe('createTmpDirWithDuplicatedTargets()', () => {
		it('Duplicates only specifically targeted .js files', async () => {
			...
			for (const e of expectedFileNames) {
				if (e.endsWith('.js')) {
					expect(dupedFileBaseNames).to.include(e, 'Unexpectedly failed to duplicate JS file');
				} else {
		...
		it('ZIPs and ZIP-type static resources are unpacked, other binary files are ignored', async () => {
			const zipPaths = [
			...
			expect(extractedRandomMeme.length).to.equal(0, 'Somehow extracted files from an image resource');
		});
	});
