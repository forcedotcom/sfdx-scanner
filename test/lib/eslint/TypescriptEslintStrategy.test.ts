	...
	describe('Test cases with tsconfig.json', () => {
		...
		describe('typescript file not in tsconfig.json causes error', () => {
			...
			it('The typescript engine should convert the eslint error to something more user friendly', async () => {
				const {results} = await ruleManager.runRulesMatchingCriteria([], ['invalid-ts'], {format: OUTPUT_FORMAT.JSON, normalizeSeverity: false, withExperimental: false, runDfa: false, sfdxVersion: 'test'}, EMPTY_ENGINE_OPTIONS);
