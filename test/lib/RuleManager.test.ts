	...
	describe('unpackTargets()', () => {
		...
		describe('Positive matching', () => {
			...
			it('Positive method-level targets are properly matched', async () => {
				// All tests will use the Sfge engine, since method-level targeting is intended for the Sfge engine anyway.
				const engine = new SfgeEngine();
				await engine.init();

				// Targets are all going to be normalized Unix paths, some of which specify method-level targets.
