	...
	describe('#recombineAndReformatResults()', () => {
		describe('Output Format: JUnit', () => {
			...
			it('Properly handles one file with one violation', () => {
				...
				const {minSev, results} = RuleResultRecombinator.recombineAndReformatResults(someFakeResults, OUTPUT_FORMAT.JUNIT);
			...
			it('Properly handles multiple files with multiple violations', () => {
				...
				const {minSev, results} = RuleResultRecombinator.recombineAndReformatResults(allFakeRuleResults, OUTPUT_FORMAT.JUNIT);
				// Split the results by newline character so we can make some interesting assertions.
				if (!isString(results)) {
					expect(false).to.equal(true, 'Results should have been string');
				} else {
