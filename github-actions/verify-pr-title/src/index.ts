...
import { verifyPrTitleMatchesTemplate } from "./verifyPrTitle";
...
function run(): void {
	try {
		...
		if (verifyPrTitleMatchesTemplate(title)) {
			console.log(`PR Title '${title}' accepted.`);
		} else {
			core.setFailed(
				`PR Title '${title}' does not match the template 'TYPE (SCOPE): @W-XXXX@: Summary'`
			);
			return;
		}
	} catch (error) {
